...
from lwx_project.utils.calculator import my_round
from lwx_project.utils.files import copy_file
from lwx_project.utils.time_obj import TimeObj


def call_excel_macro(excel_path, macro_name):
...
def get_content(df, x_range, y_range):
    if len(x_range) == 2 and len(y_range) == 2:
        ...
        data.iloc[:, 3] = data.iloc[:, 3].apply(lambda x: str(my_round(x)) if not pd.isna(x) else '')
        return data
    return df
...
def get_cheer_up_text():
    ...
    today = TimeObj()
    return TEXT_CHEER_UP.format(text_cheer_up=text_cheer_up, char_season=today.season_in_char, month_num=today.month)
